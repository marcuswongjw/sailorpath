...
- **Spreadsheet parsing appears only in admin import chunk paths** — public routes should not pull it if Import stays `next/dynamic` + `ssr: false`.
...
3. After large UI PRs, re-run `npm run analyze` and confirm spreadsheet parsing is absent from non-admin client graphs.
