  ...
  try {
    ...
    const outcome = await db.transaction(
      async (tx): Promise<ImportTransactionOutcome> => {
        ...
        for (const row of cleanRows) {
          ...
          if (!sailorId && createMissing) {
            ...
            const [createdAlias] = await db
              ...
              .returning({ aliasName: sailorAliases.aliasName });
            if (createdAlias) {
              aliasList.push({
                sailorId: createdSailor.id,
                aliasName: row.name,
          ...
          const applySail = shouldApplySailNumberFromRegatta({
            ...
            boatClass: boat,
            latestOptimistDate:
              latestOptimistDateBySailor.get(sailorId) || null,
          ...
          if (row.dob) {
            const curDob = existing?.dob
              ? String(existing.dob).slice(0, 10)
              : "";
            if (!curDob) {
        ...
        // Alias conflicts are benign; any other database failure aborts the import.
    ...
    if (matched > 0 || created > 0 || updatedProfiles > 0) {
      ...
      void logAdminChange({
        ...
        }); ${created} guests, ${updatedProfiles} profiles, ${nationalityUpdated} nationality, ${resultsDemographicsUpdated} result gender/BY stamps, ${silverUpdated} silver dates, ${removedResultRows} obsolete results removed`,
        details: {
          matched,
          created,
          updatedProfiles,
      ...
      message:
        matched === 0 && rowErrors > 0
          ...
          : `Imported ${reg.name}: ${matched}/${
              cleanRows.length
            } results saved (${created} guests auto-created, ${updatedProfiles} profiles updated when event is latest, ${nationalityUpdated} nationality from latest results, gender/birth year stamped on ${resultsDemographicsUpdated} result row(s), ${silverUpdated} silver entry dates recomputed${
              authoritativeReplace
