            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
              ...
              {hasPrivateAccess && (
                <p className="text-[10px] text-slate-500 mt-1">
                  (Private fields — visible to owner/parent/coach/superadmin)
                </p>
              )}
            </div>
          )}
      ...
      <div className="glass-panel rounded-3xl p-6 md:p-8 border border-white/5">
        ...
        <div className="space-y-4">
          {initialResults.slice(0, visibleCount).map((res: any, idx: number) => {
            ...
            const rowKey = res.id || res.regattaId || res.regattaSlug || `r-${idx}`;
            const { label, className } = getPercentileBadge(res.rank, fleetSize);
            const isExpanded = expandedRegattaId === rowKey;
            ...
            return (
              ...
              >
                {/* Main Summary Row */}
                <div
                  onClick={() => hasPrivateAccess && setExpandedRegattaId(isExpanded ? null : rowKey)}
                ...
                >
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                    <div className="text-left sm:text-right font-semibold text-sm">
                      ...
                      <span className="text-slate-400 text-xs"> / {fleetSize}</span>
                    </div>
